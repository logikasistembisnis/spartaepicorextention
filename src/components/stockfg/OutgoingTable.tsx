...
import { getOutgoingData, OutgoingItem } from '@/api/stokfg/getOutgoing'
...
export default function OutgoingTable({ period, partNum }: Props) {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            const result = await getOutgoingData(period, partNum);

            if (isMounted) {
                if (result.success && result.data) {
                    setData(result.data);
                } else {
                    setError(result.message || "Gagal mengambil detail outgoing");
                    setData([]);
                }
                setIsLoading(false);
            }
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50 sticky top-0 shadow-sm">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Mutasi Keluar</th>
                            ...
                            {data.length > 0 ? (
                                ...
                                <tr>
                                    <td colSpan={6} className="px-6 py-12 text-center text-gray-400 italic">
                                        {!partNum ? 'Pilih item di tabel stok untuk melihat detail.' : 'Tidak ada mutasi keluar untuk item ini.'}
                                    </td>
                                </tr>
                            )}
                        </tbody>
