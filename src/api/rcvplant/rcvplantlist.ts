...
export async function getRcvPlantList(
    skip = 0,
    take = 50,
    searchTerm = ""
    ...
    try {
        let filterQuery = "";
        if (searchTerm) {
            filterQuery = `&$filter=contains(UD100_Key1,'${searchTerm}')`;
        }

        const baqId = "UDGS_SearchRCPlant";
        ...
        const mappedData: RcvPlantItem[] = result.value
            .map((item) => {
                let formattedDate = "-";
