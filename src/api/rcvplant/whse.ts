...
// Fungsi menerima parameter PartNum & ShipTo untuk filtering
export async function getPartWarehouseList(
    ...
    try {
        ...
        const encodedShipTo = encodeURIComponent(shipTo);

        // Buat Query OData:
        // Filter dimana PartWhse_PartNum == inputPart DAN Warehse_Address3 == inputShipTo
        const queryFilter = `$filter=PartWhse_PartNum eq '${encodedPart}' and Warehse_Address3 eq '${encodedShipTo}'`;
